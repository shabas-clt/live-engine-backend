...
async def websocket_stream(
    websocket: WebSocket,
    asset: Optional[str] = Query(None, description="Asset to subscribe (bitcoin, gold, silver, stock_aapl, indian_stock_reliance, etc.)"),
):
    """
    ...
    if not asset:
        await websocket.send_json({
            ...
            "examples": ["bitcoin", "gold", "silver", "stock_aapl", "indian_stock_reliance"]
        })
    ...
    if asset.startswith("indian_stock_"):
    ...
    # Check if it's a US stock asset
        ...
        if asset not in valid_assets:
            await websocket.send_json({
                "error": f"Invalid asset. Must be one of: {', '.join(valid_assets)}, stock_<symbol>, or indian_stock_<symbol>",
                "examples": ["bitcoin", "gold", "silver", "stock_aapl", "indian_stock_reliance"]
            })
            await websocket.close()
            return
